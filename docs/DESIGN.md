...
`src/contracts/ai.ts` is the boundary. Vendor roles, system-message placement, tool schemas, stream event types, error bodies, and usage fields exist only in adapter files. `providers/index.ts` is the registry. Adding a provider means one adapter file plus registry and model-config entries.
...
4. **Hand-rolled vector search.** JSON vectors plus cosine similarity avoid a second service. Replace the implementation behind `searchDocuments` for production scale.
...
8. **Conservative context policy.** Estimate tokens at four characters each, reserve 15% for output, retain system messages, and remove oldest turns first.
...
- Zod validates chat/search inputs; uploads have type, count, and size limits.
...
- Virus-scan uploads, detect real file signatures, sandbox PDF parsing, and add OCR separately.
